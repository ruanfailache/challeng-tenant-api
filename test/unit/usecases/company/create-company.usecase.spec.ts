...
const MOCKED_ACTIVE_MEMBERSHIP = getMockedOwnerMembership(
  MOCKED_USER_ID,
  MOCKED_EXPECTED_COMPANY.id,
  true,
)
const MOCKED_INACTIVE_MEMBERSHIP = getMockedOwnerMembership(
  MOCKED_USER_ID,
  MOCKED_EXPECTED_COMPANY.id,
  false,
)
...
describe('CreateCompanyUseCase', () => {
  ...
  beforeEach(async () => {
    ...
    jest
      .spyOn(mockedCompanyMapper, 'fromCreateRequestToDomain')
      .mockReturnValue(MOCKED_MAPPED_COMPANY)

    jest
      .spyOn(mockedS3Service, 'uploadFile')
      .mockResolvedValue(MOCKED_UPLOAD_RESULT)

    jest
      .spyOn(mockedCompanyRepository, 'create')
      .mockResolvedValue(MOCKED_EXPECTED_COMPANY)

    jest
      .spyOn(mockedMembershipRepository, 'findActiveByUserId')
      .mockResolvedValue(null)

    jest
      .spyOn(mockedMembershipRepository, 'create')
      .mockResolvedValue(MOCKED_ACTIVE_MEMBERSHIP)

    jest
      .spyOn(mockedMembershipMapper, 'fromCreateInputToDomain')
      .mockReturnValue(MOCKED_ACTIVE_MEMBERSHIP)
  })
  ...
  it('should save the company to the repository', async () => {
    ...
    expect(mockedCompanyRepository.create).toHaveBeenCalledWith(
      MOCKED_MAPPED_COMPANY,
    )
  })
  ...
  it('should create an inactive membership if user already has an active membership', async () => {
    ...
    jest
      .spyOn(mockedMembershipMapper, 'fromCreateInputToDomain')
      .mockReturnValue(MOCKED_INACTIVE_MEMBERSHIP)

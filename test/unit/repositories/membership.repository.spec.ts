import { getMockedMembership, getMockedMembershipEntity } from '@mocks/membership.mock'
...
describe('MembershipRepository', () => {
  ...
  describe('create', () => {
    it('should create a new membership and return the domain model', async () => {
      jest.spyOn(mockedMembershipMapper, 'fromEntityToDomain').mockReturnValue(MOCKED_MEMBERSHIP)

      jest.spyOn(mockedPrismaService.membership, 'create').mockResolvedValue(MOCKED_MEMBERSHIP_ENTITY)
      ...
      expect(mockedMembershipMapper.fromEntityToDomain).toHaveBeenCalledWith(MOCKED_MEMBERSHIP_ENTITY)
  ...
  describe('findByUserId', () => {
    it('should return an array of membership domain models', async () => {
      jest.spyOn(mockedPrismaService.membership, 'findMany').mockResolvedValue([MOCKED_MEMBERSHIP_ENTITY])
    ...
    it('should return an empty array when no memberships are found', async () => {
      jest.spyOn(mockedPrismaService.membership, 'findMany').mockResolvedValue([])
  ...
  describe('findActiveByUserId', () => {
    it('should return the active membership domain model when found', async () => {
      jest.spyOn(mockedPrismaService.membership, 'findFirst').mockResolvedValue(MOCKED_MEMBERSHIP_ENTITY)
    ...
    it('should return null when no active membership is found', async () => {
      jest.spyOn(mockedPrismaService.membership, 'findFirst').mockResolvedValue(null)

      const result = await sut.findActiveByUserId('user-without-active-membership')
  ...
  describe('findByUserIdAndCompanyId', () => {
    it('should return the membership domain model when found', async () => {
      jest.spyOn(mockedPrismaService.membership, 'findUnique').mockResolvedValue(MOCKED_MEMBERSHIP_ENTITY)

      jest.spyOn(mockedMembershipMapper, 'fromEntityToDomain').mockReturnValue(MOCKED_MEMBERSHIP)

      const result = await sut.findByUserIdAndCompanyId(MOCKED_MEMBERSHIP.userId, MOCKED_MEMBERSHIP.companyId)
    ...
    it('should return null when membership is not found', async () => {
      jest.spyOn(mockedPrismaService.membership, 'findUnique').mockResolvedValue(null)

      const result = await sut.findByUserIdAndCompanyId('non-existent-user-id', 'non-existent-company-id')
  ...
  describe('deactivateAllByUserId', () => {
    it('should deactivate all active memberships for a user', async () => {
      jest.spyOn(mockedPrismaService.membership, 'updateMany').mockResolvedValue({ count: 2 })
    ...
    it('should not throw when no active memberships exist', async () => {
      jest.spyOn(mockedPrismaService.membership, 'updateMany').mockResolvedValue({ count: 0 })

      await expect(sut.deactivateAllByUserId('user-without-memberships')).resolves.not.toThrow()
  ...
  describe('activateByUserIdAndCompanyId', () => {
    it('should activate a membership and return the domain model', async () => {
      ...
      jest.spyOn(mockedPrismaService.membership, 'update').mockResolvedValue(activatedEntity)

      jest.spyOn(mockedMembershipMapper, 'fromEntityToDomain').mockReturnValue({ ...MOCKED_MEMBERSHIP, isActive: true })

      const result = await sut.activateByUserIdAndCompanyId(MOCKED_MEMBERSHIP.userId, MOCKED_MEMBERSHIP.companyId)
      ...
      expect(mockedMembershipMapper.fromEntityToDomain).toHaveBeenCalledWith(activatedEntity)
